...
// ! 404: Error: is not valid JSON need to be fixed for posts that don't exist
...
export async function generateMetadata({ params, searchParams }: IProps) {
  const dbPost = Array.from(await getPostData(params.id))[0]?.post_data
  const postData: IPostData = JSON.parse(dbPost ?? {})
