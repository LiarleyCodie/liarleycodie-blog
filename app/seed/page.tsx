...
export default async function Seed() {
    ...
    const post: IPost = {
        ...
        recent: false,
        tags: ['Cake', 'Kitchen', 'Bakery', 'Sweet'].map((tag) =>
            {
              let _tag = tag.toLowerCase()
              return _tag.replace(/^\w/, (c) => c.toUpperCase())
            },
        ),
    }
    ...
    const post2: IPost = {
        ...
        publicationDate: Math.floor(Date.now() / 1000),
        recent: false,
        tags: ['discord', 'Channel', 'Network', 'CHAT'].map((tag) => {
            let _tag = tag.toLowerCase()
            return _tag.replace(/^\w/, (c) => c.toUpperCase())
        }),
    }

    const { id, path_id } = await (await insertGridPostData(post2)).postId[0]
